...
//
//  FROM: https://github.com/daisuke310vvv/PopupController
//
...
open class PopupController: UIViewController {
        ...
        func origin(_ view: UIView, size: CGSize = UIScreen.main.bounds.size) -> CGPoint {
            ...
            case .top:
                if #available(iOS 11.0, *) {
                    return CGPoint(x: (size.width - view.frame.width) / 2, y: -64)
                } else {
                    return CGPoint(x: (size.width - view.frame.width) / 2, y: 0)
                }
            case .center:
                if #available(iOS 11.0, *) {
                    return CGPoint(x: (size.width - view.frame.width) / 2, y: ((size.height - view.frame.height) / 2) - 64)
                } else {
                    return CGPoint(x: (size.width - view.frame.width) / 2, y: (size.height - view.frame.height) / 2)
                }
            case .bottom:
                if #available(iOS 11.0, *) {
                    return CGPoint(x: (size.width - view.frame.width) / 2, y: (size.height - view.frame.height - 64))
                } else {
                    return CGPoint(x: (size.width - view.frame.width) / 2, y: size.height - view.frame.height)
                }
            }
        }
    }
    ...
    fileprivate var movesAlongWithKeyboard: Bool = false
